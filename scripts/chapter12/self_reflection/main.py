...
from scripts.chapter12.common.reflection_manager import (Reflection, ReflectionManager, TaskReflector)
...
from scripts.chapter12.passive_goal_creator.main import (Goal, PassiveGoalCreator)
from scripts.chapter12.prompt_optimizer.main import (OptimizedGoal, PromptOptimizer)
from scripts.chapter12.response_optimizer.main import ResponseOptimizer
...
def format_reflections(reflections: list[Reflection]) -> str:
    return ("\n\n".join(
        ...
        for i, r in enumerate(reflections)) if reflections else "No relevant past reflections.")
...
class ReflectiveAgentState(BaseModel):
    ...
    reflection_ids: Annotated[list[str], operator.add] = Field(default_factory=list,
...
class ResultAggregator:
    ...
    def run(
        ...
        prompt = ChatPromptTemplate.from_template("与えられた目標:\n{query}\n\n"
                                                  "調査結果:\n{results}\n\n"
...
class ReflectiveAgent:
    ...
    def _should_retry_or_continue(self, state: ReflectiveAgentState) -> str:
        ...
        if (latest_reflection and latest_reflection.judgment.needs_retry and
