...
class ReflectionJudgment(BaseModel):
    ...
    confidence: float = Field(description="あなたの判断に対するあなたの自信の度合いを0から1までの小数で示してください。")
...
class Reflection(BaseModel):
    ...
    reflection: str = Field(
        description=
        "このタスクに取り組んだ際のあなたの思考プロセスを振り返ってください。何か改善できる点はありましたか? 次に同様のタスクに取り組む際に、より良い結果を出すための教訓を2〜3文程度で簡潔に述べてください。"
    )
...
class ReflectionManager:
        ...
        with open(self.file_path, "w", encoding="utf-8") as file:
            json.dump(
                [{
                    ...
                    "embedding": embedding
                } for reflection, embedding in zip(self.reflections.values(),
    ...
    def get_relevant_reflections(self, query: str, k: int = 3) -> list[Reflection]:
        ...
        try:
            ...
            return [self.reflections[reflection_ids[i]] for i in I[0] if i < len(reflection_ids)]
